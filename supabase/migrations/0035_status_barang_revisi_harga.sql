...
begin
  ...
  if v_purchase.status_barang = 'BARANG_DITERIMA' then
    -- Tandai perubahan saat revert agar tercatat + terhapus bersama batch
    perform set_config('app.price_movement_source', 'pembelian', true);
    perform set_config('app.id_referensi', p_purchase_id::text, true);

    -- Yang teredit duluan: harga beli item pertama (paling tua) = harga sebelum pembelian
    for v_pm in
    ...
    -- Hilangkan seluruh catatan harga milik pembelian ini
